            ...
            <ul class="list">
                ...
                <li @if (config('role_manage.CrVoucher.All')==0 and
                    ...
                    <ul class="ml-menu">
                        ...
                        <!-- Delivery start -->
                        <li @if (config('role_manage.DeliveryVoucher.All')==0 and
                            config('role_manage.DeliveryVoucher.TrashShow')==0 and
                            config('role_manage.DeliveryVoucher.Create')==0) class="dis-none" @endif
                            @if (Request::url()===route('delivery_voucher') or
                            Request::url()===route('delivery_voucher.all')) class="active" @endif>
                            <a class="menu-toggle" href="javascript:void(0);">
                                <i class="fas fa-arrows-alt-h"></i>
                                <span>{{ __('root.menu.delivery') }}</span>
                            </a>

                            <ul class="ml-menu">
                                <li @if (Request::url()===route('delivery_voucher.all')) class="active" @endif>
                                    <a @if (config('role_manage.DeliveryVoucher.All')==0) class="dis-none" @endif
                                        href="{{ route('delivery_voucher.all') }}">{{ __('root.menu.delivery_all') }}</a>
                                </li>

                                <li @if (Request::url()===route('delivery_voucher')) class="active" @endif>
                                    <a @if (config('role_manage.DeliveryVoucher.Create')==0) class="dis-none" @endif
                                        href="{{ route('delivery_voucher') }}">{{ __('root.menu.delivery_create') }}</a>
                                </li>
                            </ul>
                        </li>
