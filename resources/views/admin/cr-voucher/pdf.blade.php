...
@section('content')
    ...
    <table class="table table-bordered table-sm">
        <tbody>
        <tr>
            <td>Branch Name: {{ App\Transaction::find($items[0]->id)->Branch->name }}</td>
            <td>Location: {{ App\Transaction::find($items[0]->id)->Branch->location }}</td>
            <td>Made Of Payment: {{ App\Transaction::find($items[0]->id)->BankCash->name }}</td>
    ...
    <p>In word: <span
                class="font-w-b">{{ $transaction->convert_number_to_words($total_amount) }}  {!! config('settings.currency_code') !!} </span>
        only.
    </p>
